...
int main(){
   ...
   char command[500];
	...
	for(int tsts = 0; tsts<MAX_TESTS; tsts++){
		...
		strcpy(command,"make clean");
		system(command);
		strcpy(command,"make dominion.o");
		system(command);
		strcpy(command,"make player");
		system(command);
		strcpy(command,"playdom");
		system(command);
		strcpy(command, "gcc -fprofile-arcs -ftest-coverage testdominion.c -g  dominion.o rngs.o interface.o ");
		system(command);
		...
		strcpy(command, "gcov dominion.c");
		system(command);
	...
	//for(int i=0;i<newLineChars;i++){
	//		printf("\nTPasses: %d  Fails: %d", passFailArray[i][0], passFailArray[i][1]);
	//}
