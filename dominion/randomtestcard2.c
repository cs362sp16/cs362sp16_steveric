...
int main ( int argc, char *argv[] ){
	...
	for(int i=0; i<cases; i++){
		...
		if((handAfter==handBefore) && (deckBefore == deckAfter + 1) && (actionsAfter == actionsBefore+2)){passes++;}
		else if((deckAfter == 0) && (handAfter == handBefore + deckBefore -1)&&(actionsAfter == actionsBefore+2)){passes++;}
		else{
			fails++;
